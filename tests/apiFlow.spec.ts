...
test.describe("API Flow test", () => {
    ...
    const addToCartResponseBody = await apiUtils.addToCart(userId, selectedProduct);
    expect(addToCartResponseBody.message).toBe("Product Added To Cart");

    const createOrderResponseBody = await apiUtils.createOrder(selectedProduct,"India");
    ...
    expect(getOrderDetailsBody.message).toBe("Orders fetched for customer Successfully");

    const getAllOrdersBody = await apiUtils.getAllOrders(userId);
    const order = getAllOrdersBody.data.find((order:any)=> order._id === orderId);
    ...
    const order1 = getAllOrdersBody1.data.find((order:any)=> order._id === orderId);
