...
export class OrderConfirmationPage {
    ...
    constructor(page: Page) {
        ...
        for (const key of Object.keys(orderConfirmationSelectors) as Array<keyof typeof orderConfirmationSelectors>) {
    ...
    async validateOrderConfirmationMessage(expectedMessage: string) {
        ...
        const message
    = await this.locators.orderSuccess_msg.textContent();
        if ((message?.trim() || '') !== expectedMessage) {
            throw new Error(`Expected message "${expectedMessage}", but got "${message?.trim()}"`);
        }
